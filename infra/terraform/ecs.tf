locals {
  ecr_repository_name = var.enable_create_ecr ? "${local.project_slug}" : var.existing_ecr_repository
}
...
data "aws_ecr_repository" "existing" {
  count = var.enable_create_ecr ? 0 : 1
  name  = local.ecr_repository_name
}
...
resource "aws_security_group" "lb" {
  ...
  description = "Allow inbound HTTP/HTTPS to load balancer"
  vpc_id      = aws_vpc.main.id
